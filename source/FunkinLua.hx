...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "addNegativeCamColor", function(camTarget:String)
		{
			return cameraFromString(camTarget).setFilters([new ShaderFilter(PlayState.instance.negativeStuff.shader)]);
		});
		Lua_helper.add_callback(lua, "addVcrCamEffect", function(camTarget:String)
		{
			if(ClientPrefs.lowQuality == false)
			{
				return cameraFromString(camTarget).setFilters([new ShaderFilter(PlayState.instance.vcrStuff.shader)]);
			}
		else
			{
				return;
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			...
			case 'camvfx' | 'vfx': return PlayState.instance.camVFX;
		}
