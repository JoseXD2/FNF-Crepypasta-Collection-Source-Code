...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var size:Int = 18;
		#end
		spaceText = new FlxText(textBG.x, textBG.y + 4, FlxG.width, leText, size);
		spaceText.setFormat(Paths.font("vcr.ttf"), size, FlxColor.WHITE, RIGHT);
		spaceText.scrollFactor.set();
		add(spaceText);
	...
	override function update(elapsed:Float)
	{
		...
		grpSongs.forEach(function(txt:Alphabet)
		{
			txt.screenCenter(X);
		});
		...
		if(selectedSomethin == false)
		{
			if (upP)
				...
				else if (accepted)
				{
					...
					FlxG.camera.flash(FlxColor.RED, 0.6, function()
					{
