...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['HUD Alpha', 'Value 1: the alpha (max: 1, min: 0)\nValue 2: the duration'],
		['Spawn Sonic Hand', 'spawn the sonic hand(that hole on the sonic.exe bg)'],
		['Needlemouse Jumpscare', 'spawn needlemouse\nValue 1: jumpscare duration'],
		['CHAR Alpha', 'Value 1: the character(Dad, Bf, Gf)\nValue 2: the alpha (max: 1, min: 0)'],
		['VFX Alpha', 'Value 1: the alpha (max: 1, min: 0)\nValue 2: the duration'],
		// ['GOLD trans', 'Gold transformation\nValue 1: the type of transition. 1 means when he loses his arms | 2 means when he dies lmao'],
		['Group Events', 'YOU CAN ONLY ADD 2 EVENTS\nValue 1: Event 1, this should look like this: EventName, Value1, Value2\nValue 2: Event 2, this should look like this: EventName, Value1, Value2'],
		['Flash Camera', 'makes a flash on the camera\nValue 1: the camera, the value can be [camHUD, camVFX, ALL]'],
		['Camera Funky Bop', 'start epic cam bop like on milf\nValue 1: 1 activates it | 2 disables it'],
		['Set Camera Zoom', 'put the zoom of the cam\nValue 1: zoom val, leave it blank\nif you want to put the stage zoom'],
		['Black And White', 'turns the thing black and white omg so cool so cool]\nValue 1: number 1 activates it | number 2 disables it'],
		['Raymarch Swap', 'Value 1: 0 swap the colors of camHUD and camGAME\n0 turn off | 1 turn on'],
		['Knuckles N Eggman', 'Eggman and knuckles on needlemouse song alpha\nValue 1: the alpha (max: 1, min: 0)\nValue 2: who appears[0: knuckles | 1: eggman]']
	];
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var check_events = new FlxUICheckBox(10, 50, null, null, "Enable Extern Events", 100);
		check_events.checked = _song.enableStepEvents;
		check_events.callback = function()
		{
			_song.enableStepEvents = check_events.checked;
		}
		...
		UI_author = new FlxUIInputText(saveEvents.x, saveEvents.y + 20, Std.int(saveEvents.width), '', 8);
		...
		// tab_group_song.add(check_events);
		tab_group_song.add(clear_events);
		tab_group_song.add(clear_notes);
		tab_group_song.add(saveButton);
		tab_group_song.add(saveEvents);
		tab_group_song.add(UI_author);
	...
	private function saveEvents()
	{
		...
		var eventsSong:SwagSong = {
			...
			stage: _song.stage,
			validScore: false,
			enableStepEvents: _song.enableStepEvents,
			author: _song.author
		};
		var json = {
			"song": eventsSong
